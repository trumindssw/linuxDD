...
// Function to send the current date to the client
...
int main() {
	...
	if (client_socket < 0) error_exit("Accept failed");
	 // Prepare to monitor client activity with select()
	fd_set readfds;
	...
	while (1) {
		...
		timeout.tv_usec = 0;
		// Wait for client activity with timeout
		...
		if (activity == 0) {
			...
			sleep(120);// Wait 2 minutes before shutdown
