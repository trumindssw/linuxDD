#include <stdio.h>
#include <stdlib.h>
#include <sys/ipc.h>
#include <sys/shm.h>
#include <sys/sem.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>

#define SHM_KEY 0x1234
#define SEM_KEY 0x5678
#define SHM_SIZE 1024
#define SERVER_PORT 12345
#define SERVER_IP "127.0.0.1"

union semun {
    int val;
};

int main() {
    int shmid, semid;
    char *data;
    int sockfd;
    struct sockaddr_in server_addr, client_addr;
    socklen_t client_len;

    // Create/get shared memory
    shmid = shmget(SHM_KEY, SHM_SIZE, IPC_CREAT | 0666);
    if (shmid < 0) {
        perror("shmget");
        exit(1);
    }

    // Create/get semaphore
    semid = semget(SEM_KEY, 1, IPC_CREAT | 0666);
    if (semid < 0) {
        perror("semget");
        exit(1);
    }

    // Attach shared memory
    data = (char *)shmat(shmid, NULL, 0);
    if (data == (char *)-1) {
        perror("shmat");
        exit(1);
    }

    // Set up UDP socket
    sockfd = socket(AF_INET, SOCK_DGRAM, 0);
    if (sockfd < 0) {
        perror("socket");
        exit(1);
    }

    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(SERVER_PORT);
    server_addr.sin_addr.s_addr = INADDR_ANY;  // Bind to all interfaces

    // Bind the socket so we can receive UDP messages
    if (bind(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        perror("bind");
        exit(1);
    }

    // Lock semaphore (P operation)
    struct sembuf p = {0, -1, 0};
    semop(semid, &p, 1);

    // Read from shared memory
    printf("Reader: Data read from shared memory: %s\n", data);

    // Receive data from the writer via UDP
    client_len = sizeof(client_addr);
    recvfrom(sockfd, data, SHM_SIZE, 0, (struct sockaddr *)&client_addr, &client_len);
    printf("Reader: Data received from UDP: %s\n", data);

    // Unlock semaphore (V operation)
    struct sembuf v = {0, 1, 0};
    semop(semid, &v, 1);

    // Cleanup
    shmdt(data);
    close(sockfd);

    return 0;
}

