...
int main() {
    ...
    // Bind the socket to receive UDP messages
    ...
    // Receive data from writer via UDP
    client_len = sizeof(client_addr);
    ssize_t recv_len = recvfrom(sockfd, data, SHM_SIZE, 0,
                                (struct sockaddr *)&client_addr, &client_len);
    if (recv_len < 0) {
        perror("recvfrom");
    } else {
        data[recv_len] = '\0'; // Null-terminate received data
